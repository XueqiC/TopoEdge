...
def train(data, model, classifier, loader, optimizer, loss_fn, train_edge0, prop_edge_index, args):
    ...
    for batch in loader:
        ...
        if args.method == 'none':
            ...
            if args.mixup ==0:
                # loss = (loss0 * args.reweight[data.y[idx1].view(-1)] * args.topo_reweight[idx2]).mean()
                ...
                # loss = (loss0 * args.topo_reweight[idx2]).mean()+args.h*(loss1+loss2).mean()
                # loss = (loss0 * args.topo_reweight[idx2]).mean()+args.h*(args.topo_reweight[idx2[list]]*(loss1+loss2)).mean()
        elif args.method == 'qw':
            ...
            w = args.f * args.reweight[data.y[idx1].view(-1)] + (1 - args.f) * args.topo_reweight[idx2]
            loss = ( loss0 * w ).mean()
            if args.mixup !=0:
                loss += args.h*(loss1*args.topo_reweight[idx2[list]]+loss2*args.topo_reweight[idx2_new]).mean()
                # loss += args.h*(loss1+loss2).mean()
