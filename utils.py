...
def one_sim(edge_index, edge_attr):
    ...
    for i in range(num_classes):  # Iterate over rows/classes
        for j in range(num_classes):  # Iterate over columns/dimensions
            ...
            ax = axes[i, j]
            ax.hist(class_encoding.numpy(), bins=40, color='blue', alpha=0.7, range=(0, 1))
            ax.set_title(f'Class {i} - Dim {j} - Mean: {mean_val:.4f}')

    # Adjust the layout
    plt.tight_layout()
    plt.savefig('./image/density/post_ba_loop_multi_1.png')
    plt.show()
...
def mixup(x, data, idx1, idx2, train_edge0, args):
    ...
    if args.mixup == 3: #random mixup
        lam = np.random.beta(args.alpha, args.alpha)
        edge_list = torch.arange(len(idx2))
        selected_edges = idx1
        edges_to_match = data.edge_index[:, selected_edges]
        
        connected_indices = torch.randint(0, len(train_edge0[0]), (len(idx2),))
        found_edges = train_edge0[:, connected_indices]
        matching = (data.edge_index.unsqueeze(2) == found_edges.unsqueeze(1)).all(dim=0)
        edge_list2 = matching.long().argmax(dim=0)
...
def densityplt(result):
    ...
    for cls in range(num_classes):
        for dim in range(feature_dim):
            ...
            ax.set_title(f'Dimension {dim} for Class {cls} Encoding, mean = {values.mean():.4f}')
            ax.set_xlabel('Value')
            ax.set_ylabel('Density')

    plt.tight_layout()
    plt.show()
    plt.savefig('./image/ap/post_bitcoin_alpha_multi.png')
